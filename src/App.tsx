...
import { calculateAutoLoan } from './logic/autoLoanCalculations';
import type { AutoLoanInput } from './logic/autoLoanCalculations';

function App() {
  const [values, setValues] = useState<AutoLoanInput>({
    vehiclePrice: 35000,
    downPayment: 5000,
  ...
  const handleChange = (field: keyof AutoLoanInput, value: number | boolean) => {
    setValues(prev => ({ ...prev, [field]: value }));
  };

  const result = calculateAutoLoan(values);
